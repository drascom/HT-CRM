...
function handle_users($action, $method, $db, $request_data = [])
{
    ...
    $input = $request_data;
    log_response($action, $method, $request_data);
    switch ($action) {
        case 'add':
            if ($method === 'POST') {
            ...
            if ($method === 'POST') {
                ...
                if ($id && $email && $username) {
                    ...
                    if ($password) {
                        ...
                        $stmt = $db->prepare("UPDATE users SET email = ?, username = ?, role = ?, agency_id = ?, is_active = ? WHERE id = ?");
                        $stmt->execute([$email, $username, $role, $agency_id, $is_active, $id]);
                    }
                    return ['success' => true];
                }
        ...
        case 'get':
            if ($method === 'POST') {
                $id = $input['id'] ?? null;
                if ($id) {
                    $stmt = $db->prepare("SELECT id, email, username, role, agency_id, created_at, is_active FROM users WHERE id = ?");
                    $stmt->execute([$id]);
        ...
        case 'list':
            if ($method === 'POST') {
                $stmt = $db->query("SELECT id, email, username, role, agency_id, created_at, is_active FROM users ORDER BY username");
